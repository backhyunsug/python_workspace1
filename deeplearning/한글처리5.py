...
#네이버 영화평을 준다  
#미리학습된 데이터셋을 사용한다 Word2Bec을 사용한다  
...
def study():
    ...
    embedding_dim = 300 #대충, 특별히 사전에 학습된 내용을 사용하는것이 아니면 차원을 마음대로 줄 수 있다 
    
    #limit안주면 20만개라서 메모리 fault 
    koren_word2vec_path="cc.ko.300.vec" 
    korean_word_vector = KeyedVectors.load_word2vec_format(koren_word2vec_path, binary=False, 
                                                        encoding="utf-8", limit=50000) #text파일임 
    print(korean_word_vector.vector_size)
    embedding_dim = korean_word_vector.vector_size

    #임베딩 행렬을 구축한다. 
    embedding_matrix = np.zeros( (voca_size, embedding_dim) ) #np.zero함수가 매개변수를 tuple로   그래서 괄호가 필요하다 
    #이차원 배열만들기 
    #자연어 0.2, 01, 0.2 .......................
    #감동  0.1 
    for i, word in enumerate(vocabulary): 
        if i < voca_size: #max_tokens를 벗어나지 않는 범위내에서
            try:
